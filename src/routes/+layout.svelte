<script>
    ...
    try {
      ...
      document.documentElement.style.setProperty('--secondary-color', "#f2f3f3"); // #f2f3f3 ou #000000
      document.documentElement.style.setProperty('--background-color', "#000000"); // #000000 ou white
      document.documentElement.style.color = '#000000';
      companyLogo.set("https://myvtcconnect.com/images/logo/logo_new.svg");
      /*
      document.documentElement.style.setProperty('--primary-color', "#deb320");
      document.documentElement.style.setProperty('--secondary-color', "#000000"); // #f2f3f3 ou #000000
  ...
  <Fullscreen let:onRequest let:onExit>
    <div style="background-color: var(--background-color);" class="h-fit min-h-screen">
      {#if !$fullScreen}
      <button on:click={() => (onRequest(), $fullScreen=true)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-10 h-10 inline m-5">
        <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m0-4.5L15 9m5.25 11.25h-4.5m4.5 0v-4.5m0 4.5L15 15" />
      </svg>
      </button>
      {:else }
        <button on:click={() => (onExit(), $fullScreen=false)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-10 h-10 inline m-5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M9 9V4.5M9 9H4.5M9 9L3.75 3.75M9 15v4.5M9 15H4.5M9 15l-5.25 5.25M15 9h4.5M15 9V4.5M15 9l5.25-5.25M15 15h4.5M15 15v4.5m0-4.5l5.25 5.25" />
        </svg>
        </button>
      {/if}
      <div class="grid grid-flow-row justify-center -mt-10">
        <img class="h-8 w-32 md:w-96 md:h-32" src={$companyLogo} alt="Logo">
    </div>
      <h3 class="card-title pt-5 justify-center text-center text-md font-greycliffbold -mt-5 pb-8 md:pb-12 text-transparent bg-clip-text bg-[var(--primary-color)]">{$slogan}</h3>
    
      <slot/>
      <div class="flex justify-center pb-6 bg-[var(--background-color)] pt-6 sm:pt-0">
        <footer class="footer footer-center text-base-content rounded-md w-full">
          <div class="items-center">
            <div class="text-lg -mb-5 text-center font-greycliffmed bg-clip-text text-[var(--secondary-color)]">Powered by
              <svg class= "inline w-16 h-16 pb-1" viewBox="0 0 86 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M77.1128 22.0065L72.1479 11.1583H68.027L75.1197 25.7956L74.9921 26.2364C74.8144 26.8223 74.4395 27.3282 73.9311 27.6679C73.4228 28.0076 72.8125 28.1599 72.2046 28.0989C71.493 28.0923 70.7948 27.9039 70.1761 27.5515L69.5165 30.6865C70.4684 31.0797 71.4871 31.2849 72.5167 31.2908C75.3538 31.2908 77.0702 30.2458 78.4888 27.1676L86 11.1583H82.021L77.1128 22.0065Z" fill="var(--secondary-color)"/>
                  <path d="M42.0235 5.99011H30.1219V25.9306H34.0229V18.6013H42.0235C46.3713 18.6013 49.2297 16.4047 49.2297 12.2957C49.2297 8.18677 46.3713 5.99011 42.0235 5.99011ZM41.8107 15.1109H34.0087V9.42372H41.8107C44.0662 9.42372 45.357 10.4545 45.357 12.2673C45.357 14.0801 44.0662 15.1109 41.8107 15.1109Z" fill="var(--secondary-color)"/>
                  <path d="M65.539 22.1487V16.1416C65.539 12.5872 62.9928 10.746 58.6236 10.746C55.0773 10.746 51.9706 12.4024 51.0982 14.9473L54.3042 16.0848C54.7794 14.8123 56.432 13.8739 58.4889 13.8739C60.9288 13.8739 61.9572 14.8691 61.9572 16.0848V16.4758L56.1554 17.1156C52.8147 17.471 50.6159 18.971 50.6159 21.6511C50.6159 24.587 53.1339 26.1652 56.4745 26.1652C58.6278 26.2325 60.7248 25.4691 62.3331 24.0325C62.9147 25.4543 63.5105 26.4069 67.4754 25.9022V22.9307C65.8866 23.3145 65.539 22.9307 65.539 22.1487ZM61.9927 20.2435C61.9927 22.1771 59.2903 23.2008 57.0278 23.2008C55.3042 23.2008 54.2687 22.6463 54.2687 21.5444C54.2687 20.4425 55.1198 20.0444 56.7653 19.8525L62.0069 19.2411L61.9927 20.2435Z" fill="var(--secondary-color)"/>
            ...
            <span class="text-sm text-[var(--secondary-color)]">{$companyName} © {$companyStartYear} - {new Date().getFullYear()} Tous droits réservés</span>
          </div>
        </footer>
      </div>
      </div>     
